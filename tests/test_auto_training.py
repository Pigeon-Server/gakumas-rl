...
from src.training.backends import (
    TrainingResult,
    TrainingSpec,
...
def test_safe_save_training_metadata_does_not_raise_on_failure() -> None:
    ...
    import src.training.backends as backends_module
...
def test_generate_demo_artifacts_for_exam_training() -> None:
    ...
    fake_module = ModuleType('src.demo_exam')
    ...
    original_module = sys.modules.get('src.demo_exam')
    sys.modules['src.demo_exam'] = fake_module
    try:
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
        ...
        if original_module is None:
            sys.modules.pop('src.demo_exam', None)
        else:
            sys.modules['src.demo_exam'] = original_module
